...
class ParallelBundleAdjustmentSolver{
	...
	double lm_previous_error;
	double lm_current_error;
        double lm_damping_ratio;
	double lm_damping_scaling;
	double lm_gain_ratio_division;
	double lm_min_damping;
	double lm_max_damping;
	...
	public:
        ...
        void SetMSEThreshold(double threshold);
	    void GetAugmentedHessian();
        ...
        void dumpSubsetOfHessian(std::vector<ObjectPoint>& vObjectPointList, std::vector<Projection>& vProjectionList, std::vector<Camera>& vCameraList);
